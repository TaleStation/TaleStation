...
/obj/effect/sunlight/proc/warn_daylight(danger_level = 0, vampwarn = "", vassalwarn = "", hunteralert = "")
	for(var/datum/mind/bloodsucker_minds as anything in get_antag_minds(/datum/antagonist/bloodsucker))
		...
		if(bloodsucker_minds.current)
			switch(danger_level)
				if(1)
					bloodsucker_minds.current.playsound_local(null, 'jollystation_modules/code/modules/antagonists/bloodsuckers/sounds/griffin_3.ogg', 50 + danger_level, 1)
				if(2)
					bloodsucker_minds.current.playsound_local(null, 'jollystation_modules/code/modules/antagonists/bloodsuckers/sounds/griffin_5.ogg', 50 + danger_level, 1)
				if(3)
