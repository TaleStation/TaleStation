...
/datum/advanced_antag_datum/traitor/infiltrator/greet_message(mob/antagonist)
	to_chat(antagonist, "<span class='alertsyndie'>You are an [name]!</span>")
...
/datum/advanced_antag_datum/traitor/infiltrator/greet_message_two(mob/antagonist)
	to_chat(antagonist, "<span class='danger'>You are an agent sent to infiltrate [station_name()]! You can set your goals to whatever you think would make an interesting story or round. You have access to your goal panel via verb in your IC tab.</span>")
...
/datum/advanced_antag_datum/traitor/infiltrator/podspawn/greet_message_two(mob/antagonist)
	to_chat(antagonist, "<span class='danger'>You are an agent preparing to infiltrate [station_name()]! You can set your goals to whatever you think would make an interesting story or round. Finalizing your goals will drop you into a random maintenance room on the station. You have access to your goal panel via verb in your IC tab.</span>")
	addtimer(CALLBACK(src, .proc/greet_message_three, antagonist), 3 SECONDS)
