...
/obj/structure/destructible/cult/item_dispenser/daemon_forge/Initialize(mapload)
	...
	if(!glow)
		glow = mutable_appearance(icon, "tinkerglow", layer = FLOAT_LAYER, offset_spokesman = src, plane = EMISSIVE_PLANE)
		glow.color = COLOR_YELLOW
		glow.overlays += emissive_appearance(icon, "tinkerglow", offset_spokesman = glow, layer = FLOAT_LAYER, alpha = glow.alpha)
	add_overlay(glow)
...
/obj/structure/destructible/cult/item_dispenser/daemon_forge/update_overlays()
	...
	if(!glow)
		glow = mutable_appearance(icon, "tinkerglow", offset_spokesman = src, layer = FLOAT_LAYER, plane = EMISSIVE_PLANE, alpha = 175)
		glow.color = COLOR_YELLOW
		glow.overlays += emissive_appearance(icon, "tinkerglow", offset_spokesman = glow, layer = FLOAT_LAYER, alpha = glow.alpha)
...
/obj/structure/destructible/cult/item_dispenser/daemon_forge/setup_options()
	var/static/list/clock_forge_items = list(
		...
		BRASS_ARMOR = list(
			PREVIEW_IMAGE = image(icon = 'talestation_modules/icons/mob/actions/actions_clockcult.dmi', icon_state = "clockwork_armor"),
