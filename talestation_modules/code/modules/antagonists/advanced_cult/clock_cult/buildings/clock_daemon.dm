...
/obj/structure/destructible/cult/item_dispenser/daemon_forge/Initialize(mapload)
	...
	if(!glow)
		glow = mutable_appearance(icon, "tinkerglow", layer = FLOAT_LAYER, plane = EMISSIVE_PLANE)
		glow.color = COLOR_YELLOW
		glow.overlays += emissive_appearance(icon, "tinkerglow", glow, layer = FLOAT_LAYER, alpha = glow.alpha)
	add_overlay(glow)
...
/obj/structure/destructible/cult/item_dispenser/daemon_forge/update_overlays()
	...
	if(!glow)
		glow = mutable_appearance(icon, "tinkerglow", layer = FLOAT_LAYER, plane = EMISSIVE_PLANE, alpha = 175)
		glow.color = COLOR_YELLOW
		glow.overlays += emissive_appearance(icon, "tinkerglow", glow, layer = FLOAT_LAYER, alpha = glow.alpha)
