...
/datum/controller/subsystem/ticker/fire()
	switch(current_state)
		if(GAME_STATE_STARTUP)
			...
			send2chat(new /datum/tgs_message_content("<@&[CONFIG_GET(string/ping_role_id)]> Round **[GLOB.round_id]** starting on [SSmapping.config.map_name]."), CONFIG_GET(string/channel_announce_new_game))
...
/datum/controller/subsystem/ticker/proc/equip_characters()
	...
	if(captainless)
		for(var/mob/dead/new_player/new_player_mob as anything in GLOB.new_player_list)
			var/mob/living/carbon/human/new_player_human = new_player_mob.new_character
			if(new_player_human)
				to_chat(new_player_mob, span_notice("Leadership not forced on anyone."))
			CHECK_TICK
